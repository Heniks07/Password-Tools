...
namespace PasswordTools
{
    ...
    class Program
    {
        ...
        static void Input()
        {
            ...
            if (input == "?")
            {
                Console.WriteLine("___Help___\n\n" +
                    ...
                    " logicPswdC\topens the logic password Cracker\n");
                return;
            }
            ...
            if (input == "lpc")
            {
                LogicPasswordCracker logicPswdC = new LogicPasswordCracker();
                logicPswdC.Run();
            }
            if (input == "clear")
            {
        ...
        static void Run()
        {
            ...
            {
                Console.WriteLine("Welcome to this application! You can find infos and help on my githubt: https://github.com/Heniks07/Password-Tools");
            ...
            if (string.IsNullOrEmpty(configuration.CrackGenerationsTemporary))
            {
                Console.Write("Please input the path where your logic password cracker generations should be temporary stored\n>");
                string input = Console.ReadLine();
                if (!Directory.Exists(input))
                {
                    Console.WriteLine("This is not a valid Path!");
                ...
                configuration.CrackGenerationsTemporary = input + "\\lpc-temp.txt";

                File.Create(configuration.CrackGenerationsTemporary).Close();
                File.WriteAllText(configuration.CrackGenerationsTemporary, "[]");
            }
            if (configuration.LPCDelimiters == null)
            {
                configuration.LPCDelimiters = new List<char>() { '.', '-', '_', '+', '~' };
    ...
    public class Configuration
    {
        ...
        public string CrackGenerationsTemporary { get; set; }
        public List<char> LPCDelimiters { get; set; }
    }
}
